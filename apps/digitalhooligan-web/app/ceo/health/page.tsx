...
function safeParseWhy(raw: string | string[] | undefined): WhyPayload | null {
...
export default async function Page({
    ...
    const appsJson: unknown = appsRes.ok ? await appsRes.json() : null;
    const incJson: unknown = incRes.ok ? await incRes.json() : null;
    ...
    const why = safeParseWhy(searchParams?.why);
    const showWhy =
        Boolean(why) &&
            ...
            <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="mt-1 text-sm text-white/60">
                        ...
                        <code className="text-white/80">/api/incidents</code>. Refresh the page to re-check.
                    </p>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <div className="mt-3 space-y-2">
                        ...
                        {apps.map((a, idx) => {
                            ...
                            return (
                                <div key={`${name}-${idx}`} className="flex items-center justify-between rounded-xl border border-white/10 bg-black/20 px-3 py-2">
                                    <div className="text-sm text-white/85">{name}</div>
                                    <span className={badgeClasses(bucket)}>
                                        {bucket === 'healthy' ? 'healthy' : bucket}
                                    </span>
                                </div>
                            );
                        })}
