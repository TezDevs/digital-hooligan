      ...
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-8 md:pt-10">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/70">
                    ...
                    <div className="flex flex-col gap-1 md:flex-row md:items-start md:justify-between">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="text-sm font-medium text-slate-50">{exp.title}</span>
                          <span className={statusClasses(exp.status)}>{STATUS_LABEL[exp.status]}</span>
                          <span className={impactClasses(exp.impact)}>{IMPACT_LABEL[exp.impact]}</span>
                        </div>
                        <p className="mt-1 text-[0.8rem] text-slate-300">{exp.summary}</p>
                        <div className="mt-1 flex flex-wrap gap-2 text-[0.7rem] text-slate-400">
                          ...
                          {exp.tags && exp.tags.length > 0 && (
                            <span>
                              Tags: <span className="text-slate-300">{exp.tags.join(", ")}</span>
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <div className="border-t border-slate-800 px-4 py-2 text-[0.7rem] text-slate-500">
            Snapshot timestamp: {lastUpdated ? lastUpdated : "not tracked yet. (Mock data)"}
          </div>
        </div>
      </div>
    </main>
